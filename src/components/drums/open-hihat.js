...
export default function OpenHihat({ohStruct, setOhStruct, steps}) {
    ...
    function toggleNote(index) {
        setStruct(prev => {
            const newStr = [...prev];
            newStr[index] = prev[index] === "oh" ? "~" : "oh";
            return newStr;
        });
    }    
    // update bass struct when toggleNote is called
    useEffect(() => {
        setOhStruct(struct.join(" "));
    }, [struct]);
    ...
    useEffect(() => {
        ...
        // existing notes
        rects.attr("fill", d => (d.note === "oh" ? "white" : "#171717ff"));
        ...
        .attr("width", width)
        .attr("height", height)
        .attr("fill", d => (d.note === "oh" ? "white" : "#171717ff"))
