...
export default function RimShot({rimStruct, setRimStruct, steps}) {
    ...
    function toggleNote(index) {
        setStruct(prev => {
            const newStr = [...prev];
            newStr[index] = prev[index] === "rim" ? "~" : "rim";
            return newStr;
        });
    }    
    // update bass struct when toggleNote is called
    useEffect(() => {
        setRimStruct(struct.join(" "));
    }, [struct]);
    ...
    useEffect(() => {
        ...
        // existing notes
        rects.attr("fill", d => (d.note === "rim" ? "white" : "#171717ff"));
        ...
        .attr("width", width)
        .attr("height", height)
        .attr("fill", d => (d.note === "rim" ? "white" : "#171717ff"))
