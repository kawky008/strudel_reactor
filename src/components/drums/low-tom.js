...
export default function LowTom({ltStruct, setLtStruct, steps}) {
    ...
    function toggleNote(index) {
        setStruct(prev => {
            const newStr = [...prev];
            newStr[index] = prev[index] === "lt" ? "~" : "lt";
            return newStr;
        });
    }    
    // update bass struct when toggleNote is called
    useEffect(() => {
        setLtStruct(struct.join(" "));
    }, [struct]);
    ...
    useEffect(() => {
        ...
        // existing notes
        rects.attr("fill", d => (d.note === "lt" ? "white" : "#171717ff"));
        ...
        .attr("width", width)
        .attr("height", height)
        .attr("fill", d => (d.note === "lt" ? "white" : "#171717ff"))
