...
export default function CrashCymbal({crStruct, setCrStruct, steps}) {
    ...
    function toggleNote(index) {
        setStruct(prev => {
            const newStr = [...prev];
            newStr[index] = prev[index] === "cr" ? "~" : "cr";
            return newStr;
        });
    }    
    // update bass struct when toggleNote is called
    useEffect(() => {
        setCrStruct(struct.join(" "));
    }, [struct]);
    ...
    useEffect(() => {
        ...
        // existing notes
        rects.attr("fill", d => (d.note === "cr" ? "white" : "#171717ff"));
        ...
        .attr("width", width)
        .attr("height", height)
        .attr("fill", d => (d.note === "cr" ? "white" : "#171717ff"))
