...
export default function HighTom({htStruct, setHtStruct, steps}) {
    ...
    function toggleNote(index) {
        setStruct(prev => {
            const newStr = [...prev];
            newStr[index] = prev[index] === "ht" ? "~" : "ht";
            return newStr;
        });
    }    
    // update bass struct when toggleNote is called
    useEffect(() => {
        setHtStruct(struct.join(" "));
    }, [struct]);
    ...
    useEffect(() => {
        ...
        // existing notes
        rects.attr("fill", d => (d.note === "ht" ? "white" : "#171717ff"));
        ...
        .attr("width", width)
        .attr("height", height)
        .attr("fill", d => (d.note === "ht" ? "white" : "#171717ff"))
