...
export default function RideCymbal({rdStruct, setRdStruct, steps}) {
    ...
    function toggleNote(index) {
        setStruct(prev => {
            const newStr = [...prev];
            newStr[index] = prev[index] === "rd" ? "~" : "rd";
            return newStr;
        });
    }    
    // update bass struct when toggleNote is called
    useEffect(() => {
        setRdStruct(struct.join(" "));
    }, [struct]);
    ...
    useEffect(() => {
        ...
        // existing notes
        rects.attr("fill", d => (d.note === "rd" ? "white" : "#171717ff"));
        ...
        .attr("width", width)
        .attr("height", height)
        .attr("fill", d => (d.note === "rd" ? "white" : "#171717ff"))
