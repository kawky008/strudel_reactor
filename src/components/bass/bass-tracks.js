import { useBassStore } from "../../stores/use-bass-store.js";
import GeneralVolumeSlider from "../volume-sliders/general-volume-slider.js";

export default function BassTrack({
    ...
    const resetTrack = useBassStore((state) => state.resetTrack);
        ...
        <div style={{ display: "flex", gap: "1rem"}}>
            <div className="track-settings">
                ...
                <div
                    ...
                    onClick={() => updateBass(name, { play: !play })}
                >
                    {play ? <i className="fa-solid fa-volume-high" /> : <i className="fa-solid fa-volume-xmark" /> }
                </div>

                {/* volume slider */}
                <GeneralVolumeSlider name={name} gain={gain} update={updateBass} />
